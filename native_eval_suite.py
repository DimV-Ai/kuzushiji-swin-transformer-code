#!/usr/bin/env python3
"""
Native evaluation suite for the 6 stratified kuzushiji classifiers.

What it does
------------
For each model, it rebuilds the *native* evaluation split in the same way as the
...
Usage
-----
Edit MODEL_SPECS below, then run:
  python native_eval_suite.py

or with a custom output root:
  python native_eval_suite.py --out_root /path/to/out
"""
...
MODEL_SPECS = [
    {
        "name": "ogihan_min10",
        "dataset_kind": "ogihan_only",
        "min_count": 10,
        "model_dir": "/home/mdxuser/datasets_and_models/kuzushiji_class_models/ogihan_only/min10",
    },
    {
        "name": "ogihan_min20",
        "dataset_kind": "ogihan_only",
        "min_count": 20,
        "model_dir": "/home/mdxuser/datasets_and_models/kuzushiji_class_models/ogihan_only_20/min20",
    },
    {
        "name": "codh_min10",
        "dataset_kind": "codh_only",
        "min_count": 10,
        "model_dir": "/home/mdxuser/datasets_and_models/kuzushiji_class_models/codh_only/min10",
    ...
    },
    {
        ...
        "dataset_kind": "codh_only",
        "min_count": 20,
        "model_dir": "/home/mdxuser/datasets_and_models/kuzushiji_class_models/codh_only/min20",
        "dataset_dir": "/home/mdxuser/datasets_and_models/kuzushiji_classifier_data_codh",
    },
]
...
def prepare_native_split(spec: dict) -> Tuple[Dataset, Dict[str, int], Dict[int, str], str]:
    ...
    if kind == "ogihan_only":
        ds = build_ogihan_dataset()
        label_column = "unicode"
    elif kind == "codh_only":
        ds = build_codh_only_dataset(spec["dataset_dir"])
        label_column = "char"
    else:
    ...
    ds = ds.map(
        lambda x: {"labels": label2id[x]},
        input_columns=[label_column],
        desc=f"Encoding labels from {label_column}",
    )
    ...
    ds = ds.filter(
        lambda lab: lab in keep_old_ids_set,
        input_columns=["labels"],
        desc="Filtering classes by minimum support",
    )

    old_id_to_new_id = {old: new for new, old in enumerate(keep_old_ids)}

    ds = ds.map(
        lambda lab: {"labels": old_id_to_new_id[lab]},
        input_columns=["labels"],
        desc="Remapping filtered labels",
    )
    ...
    # datasets.train_test_split(..., stratify_by_column=...) requires a ClassLabel feature.
    ds = ds.cast_column("labels", ClassLabel(num_classes=len(kept_names), names=kept_names))

    split = ds.train_test_split(test_size=TEST_SIZE, seed=SPLIT_SEED, stratify_by_column="labels")
    eval_ds = split["test"]
...
def main():
    ...
    for spec in MODEL_SPECS:
        ...
        if model_label_keys != rebuilt_keys:
            ...
            raise ValueError(
                f"Label-space mismatch for {name}. "
                f"Rebuilt-only labels: {len(missing_in_model)}; "
                f"Model-only labels: {len(missing_in_rebuilt)}. "
                f"See rebuilt_label_space.csv, model_label_space.csv, and label_space_diff.csv in {eval_dir}"
            )
        ...
        eval_ds = eval_ds.map(relabel_for_model)
    ...
    summary_df = summary_df[[
        ...
        "top5_accuracy", "total_errors", "model_dir",
    ]]
