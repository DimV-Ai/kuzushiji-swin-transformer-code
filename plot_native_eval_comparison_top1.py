...
def load_and_combine_summaries(paths: List[str]) -> pd.DataFrame:
    ...
    combined = add_top1_columns(combined)
    return combined
...
def add_top1_columns(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    if "top1_accuracy" not in df.columns:
        df["top1_accuracy"] = df["accuracy"]
    return df
...
def save_tables(summary_df: pd.DataFrame, out_dir: Path, valid_only_df: pd.DataFrame) -> None:
    raw_cols = [
        ...
        "accuracy", "top1_accuracy", "precision_macro", "recall_macro", "f1_macro",
    ...
    present_cols = [
        ...
        "accuracy", "top1_accuracy", "recall_macro", "f1_macro", "top5_accuracy",
    ...
    rank_df = valid_only_df[
        ["display_name", "regime", "min_count", "accuracy", "top1_accuracy", "recall_macro", "f1_macro", "top5_accuracy"]
    ].copy()
...
def plot_grouped_main_metrics(summary_df: pd.DataFrame, out_path: Path) -> None:
    ...
    metrics = ["top1_accuracy", "recall_macro", "f1_macro", "top5_accuracy"]
    width = 0.18
    x = list(range(len(df)))
...
def main() -> None:
    ...
    plot_metric_bar(valid_only_df, "top1_accuracy", "Top-1 accuracy across valid models", out_dir / "top1_accuracy_valid_models.png")
